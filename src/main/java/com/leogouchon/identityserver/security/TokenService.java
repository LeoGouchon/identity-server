...
public class TokenService {
    ...
    public TokenService(@Value("${identity.issuer}") String issuer, @Value("${identity.access-token-ttl-seconds}") long accessTtl, SigningKeyRepository signingKeys) {
        this.issuer = issuer; this.accessTtl = accessTtl; this.signingKeys = signingKeys;
    }
    @PostConstruct
    void initializeKey() throws Exception {
        ...
        if (existing.isPresent()) {
            ...
            keyPair = new KeyPair(publicKey, privateKey); return;
        }
        KeyPairGenerator generator = KeyPairGenerator.getInstance("RSA"); generator.initialize(2048); keyPair = generator.generateKeyPair();
        SigningKey key = new SigningKey(); key.setId(java.util.UUID.randomUUID()); key.setPrivateKey(java.util.Base64.getEncoder().encodeToString(keyPair.getPrivate().getEncoded())); key.setPublicKey(java.util.Base64.getEncoder().encodeToString(keyPair.getPublic().getEncoded())); signingKeys.save(key);
    }
